    ...
    while (1) {
        ...
        if (strcmp(alg, "FCFS") == 0) {
            ...
            if (outmode == 2 || outmode == 3)
                if (oflag)
                    fprintf(fp2, "time=%ld, cpu=%d, pid=%d, burstlen=%d, remainingtime=%d\n",
            ...
            if (outmode == 3)
                if (oflag)
                    fprintf(fp2, "time=%ld. Burst finished. cpu=%d, pid=%d, burstlen=%d, remainingtime=%d\n",
                        (end.tv_sec - start.tv_sec) * 1000 + (end.tv_usec - start.tv_usec) / 1000, b->processor_id, b->pid, b->burst_length, b->remaining_time);
                else
...
int main(int argc, char *argv[]) {
        ...
        if (sscanf(buffer, "PL %d\n", &pl) == 1) {
            ...
            if (strcmp(sap, "M") == 0) {
                if (strcmp(qs, "RM") == 0) {
                    ...
                    if (outmode == 3)
                        if (oflag)
                            fprintf(fp2, "time=%ld. Add process %d to queue %d. Burst Length: %d\n",
    ...
    if (oflag)
    {
        fprintf(fp2, "pid\tcpu\tburstlen\tarv\tfinish\twaitingtime\tturnaround\n");
        for (int i = 0; i < get_length(&analysis_list); i++) {
            fprintf(fp2, "%d\t%d\t%d\t\t%ld\t%ld\t%d\t\t%d\n", 
        ...
        fprintf(fp2, "Average Turnaround Time: %f\n", get_avg_turnaround(&analysis_list));
        fclose(fp2);
    }
    else {
